...
import { Modal, Image, TouchableHighlight, TouchableOpacity, TextInput, View, Text, Button, ScrollView, StyleSheet} from 'react-native';
...
export default class MenuScreen extends Component {
	constructor(props) {
		...
		this.manageInventoryModalVisible = this.manageInventoryModalVisible.bind(this);
	}
	...
	addItem = () => {
		...
		if((this.state.mealName == "")||(this.state.picture == "")||(this.state.cost == "")||(this.state.availability == "")) {
	...
	getAddButton = () => {
		...
		if(this.props.role == "Admin") {
			...
			<Text style={styles.buttonText} onPress={()=>{this.setModalVisible();}}>
			  Add Item
			</Text>
	  		</TouchableOpacity>
			)
	...
	getUpdateButton = (MealName, Picture, Cost, Availability) => {
		if(this.props.role == "Admin") {
			return(<TouchableOpacity style={[styles.updateButton]}> 
				<Text style={styles.buttonText} onPress={()=>{this.setEditModalVisible(MealName, Picture, Cost, Availability);}}>
				  Update Item
				</Text>
				  </TouchableOpacity>
				)
		...
		if(this.props.role == "Staff") {
			return (<TouchableOpacity style={[styles.updateButton]}> 
			<Text style={styles.buttonText} onPress={()=>{this.setInventoryModalVisible(MealName, Picture, Cost, Availability);}}>
			  Update Item Status
			</Text>
  			</TouchableOpacity>
			)
	...
	getBuyButton = (mealID, mealName, availability) => {
		if(this.props.role == "Customer") {
			return (<TouchableOpacity style={[styles.updateButton]}> 
				<Text style={styles.buttonText} onPress={()=>{this.addItemToOrder(mealID, mealName, availability);}}>
				  Add to order
				</Text>
				  </TouchableOpacity>
				)
	...
	render() {
		const {	modalVisible, editModalVisible, inventoryModalVisible} = this.state;
            ...
            this.state.items.map((x, i) => (
              ...
              <ListItem key={i} bottomDivider>
				...
				{/* <Button  style={{borderRadius:50}} color="orange" title="Update Item" onPress={()=>{this.setEditModalVisible(!editModalVisible, x.MealName, x.Picture, x.Cost, x.Availability);}}/> */}
					...
					{this.getUpdateButton(x.MealName, x.Picture, x.Cost, x.Availability)}
